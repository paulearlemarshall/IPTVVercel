...
export function checkLocalHelper() {
  if (pending) return pending;
  publish({ ...snapshot, status: "checking", detail: "Checking the local helper…" });
  pending = (async () => {
...
function visibleCheck() { if (document.visibilityState === "visible") void checkLocalHelper(); }
function subscribe(listener: () => void) {
  listeners.add(listener);
  if (listeners.size === 1) {
    visibleCheck();
    timer = setInterval(visibleCheck, 15000);
...
export function useLocalHelperStatus() {
