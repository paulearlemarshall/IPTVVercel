...
The top left of the main window and player shows shared local-helper status: green **Helper ready**, red **unreachable/needs attention**, amber **update needed**, or neutral **checking**. Hover or keyboard-focus shows IP/port, check time and latency, encoders, runtime/system summary and session usage; Retry refreshes it. One shared poll runs every 15 seconds while the page is visible, with an eight-second timeout. A blocked request is reported as unreachable or permission blocked, not as proof the helper is uninstalled.
...
Click the indicator to open `http://127.0.0.1:19876/` on the viewing PC. This read-only HTML page explains operation and troubleshooting and displays safe system details, allowed origins/provider hosts, readiness and sessions. It refreshes every 15 seconds, supports light/dark themes and excludes credentials, stream URLs, session tokens, machine/user names and home paths. Only this page accepts navigation without an Origin header; Host validation remains mandatory, API Origin checks remain enforced, and framing/scripts/forms are blocked by security headers. If the helper is stopped this local page cannot load. Restart an already-running helper after updating its code to enable the new page and health response. No new environment variables are required.
