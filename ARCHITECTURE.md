...
`LocalHelperStatus` appears on the right of both headers. A shared store polls health every 15 seconds while visible, about two seconds on hover/focus, with coalescing and an eight-second timeout. `version:1`, `healthSchema:2`, and additive `telemetrySchema:1` expose readiness and sanitized measurements. `telemetry.mjs` counts stdout bytes through a Transform and parses bounded stderr progress/codec metadata without retaining raw logs or opening extra provider connections. No source paths/queries or session IDs are exposed. `/status.js` updates the read-only local dashboard from `/status` at selectable intervals with refresh/pause controls. HTML/static scripts have a no-Origin exception; `/status` requires an approved Origin or missing Origin with same-origin Fetch Metadata. `/health` and playback APIs retain strict Origin checks. README and AI helper specifications document measurement limits and contracts.
